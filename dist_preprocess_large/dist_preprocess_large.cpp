...
long long int Level[MAX];  //level of each node
...
void contract(long long int v)
{
    ...
    long long int out_max=0; 
     ...
     for(auto jj:adj[v])
     {
         ...
         Level[jj.second]=max(Level[jj.second],Level[v]+1);
    ...
    for(auto jj:adjr[v])
    {
        ...
        for(auto kk:adj[v])
        {
            ...
            {
                adj[jj.second].pb({cost,kk.second});
...
void preprocessing()
{
    ...
    {   worksetD.insert(i);
        workset.insert(i);
        long long  imp = 14*((adj[i].size()*adjr[i].size())-adj[i].size()-adjr[i].size())+25*(adj[i].size()+adjr[i].size());
        imp+= 20*delNeighbors[i]+50*Level[i];
        imp_pq.push({imp,i});
     }
